...
if ( ! function_exists( 'rrr_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time and author.
	 */
	function rrr_posted_on() {
		...
		$posted_on = sprintf(
			esc_html_x( 'Posted on %s', 'post date', 'rrr' ),
			'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
		);

		$byline = sprintf(
			esc_html_x( 'by %s', 'post author', 'rrr' ),
...
if ( ! function_exists( 'rrr_entry_footer' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 */
	function rrr_entry_footer() {
		// Hide category and tag text for pages.
		if ( 'post' === get_post_type() ) {
			/* translators: used between list items, there is a space after the comma */
			$categories_list = get_the_category_list( esc_html__( ', ', 'rrr' ) );
			if ( $categories_list && rrr_categorized_blog() ) {
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'rrr' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html__( ', ', 'rrr' ) );
			if ( $tags_list ) {
 ...
 * @return bool
 */
function rrr_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'rrr_categories' ) ) ) {
		...
		set_transient( 'rrr_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so rrr_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so rrr_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in rrr_categorized_blog.
 */
function rrr_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return false;
	}
	// Like, beat it. Dig?
	delete_transient( 'rrr_categories' );
}
add_action( 'delete_category', 'rrr_category_transient_flusher' );
add_action( 'save_post',     'rrr_category_transient_flusher' );
 ...
 */
function rrr_get_svg( $args = array() ) {

	// Make sure $args are an array.
	if ( empty( $args ) ) {
		return esc_html__( 'Please define default parameters in the form of an array.', 'rrr' );
	}

	// YUNO define an icon?
	if ( false === array_key_exists( 'icon', $args ) ) {
		return esc_html__( 'Please define an SVG icon filename.', 'rrr' );
	}
 ...
 */
function rrr_get_the_title( $args = array() ) {
 ...
 * @return string        The shortened excerpt.
 */
function rrr_get_the_excerpt( $args = array() ) {
 ...
 */
function rrr_do_post_image( $size = 'thumbnail' ) {
 ...
 */
function rrr_get_post_image_uri( $size = 'thumbnail' ) {
 ...
 */
function rrr_get_attachment_id_from_url( $attachment_url = '' ) {
 ...
 */
function rrr_do_copyright_text() {

	// Grab our customizer settings.
	$copyright_text = get_theme_mod( 'rrr_copyright_text' );
...
function rrr_get_social_share() {
	...
	<div class="social-share">
		<h5 class="social-share-title"><?php esc_html_e( 'Share This', 'rrr' ); ?></h5>
		<ul class="social-icons menu menu-horizontal">
			<li class="social-icon">
				<a href="<?php echo esc_url( $twitter_url ); ?>" onclick="window.open(this.href, 'targetWindow', 'toolbar=no, location=no, status=no, menubar=no, scrollbars=yes, resizable=yes, top=150, left=0, width=600, height=300' ); return false;">
					<?php rrr_do_svg( array( 'icon' => 'twitter-square', 'title' => 'Twitter', 'desc' => __( 'Share on Twitter', 'rrr' ) ) ); ?>
					<span class="screen-reader-text"><?php esc_html_e( 'Share on Twitter', 'rrr' ); ?></span>
				</a>
			</li>
			<li class="social-icon">
				<a href="<?php echo esc_url( $facebook_url ); ?>" onclick="window.open(this.href, 'targetWindow', 'toolbar=no, location=no, status=no, menubar=no, scrollbars=yes, resizable=yes, top=150, left=0, width=600, height=300' ); return false;">
					<?php rrr_do_svg( array( 'icon' => 'facebook-square', 'title' => 'Facebook', 'desc' => __( 'Share on Facebook', 'rrr' ) ) ); ?>
					<span class="screen-reader-text"><?php esc_html_e( 'Share on Facebook', 'rrr' ); ?></span>
				</a>
			</li>
			<li class="social-icon">
				<a href="<?php echo esc_url( $linkedin_url ); ?>" onclick="window.open(this.href, 'targetWindow', 'toolbar=no, location=no, status=no, menubar=no, scrollbars=yes, resizable=yes, top=150, left=0, width=475, height=505' ); return false;">
					<?php rrr_do_svg( array( 'icon' => 'linkedin-square', 'title' => 'LinkedIn', 'desc' => __( 'Share on LinkedIn', 'rrr' ) ) ); ?>
					<span class="screen-reader-text"><?php esc_html_e( 'Share on LinkedIn', 'rrr' ); ?></span>
				</a>
			</li>
		</ul>
	</div><!-- .social-share -->
