...
gulp.task('clean:pot', function() {
	return del(['languages/run-rabbit-run.pot']);
});
...
gulp.task('wp-pot', ['clean:pot'], function() {
	...
	.pipe(wpPot({
		domain: 'rrr',
		destFile:'run-rabbit-run.pot',
		package: 'rrr',
		bugReport: 'http://http://localhost:8888/run-rabbit',
		lastTranslator: 'John Doe <mail@http://localhost:8888/run-rabbit>',
		team: 'Team <mail@http://localhost:8888/run-rabbit>'
	}))
...
gulp.task('watch', function() {
	...
	browserSync({
		...
		proxy: "http://localhost:8888/run-rabbit:3000",         // Use http://localhost:8888/run-rabbit:3000 to use BrowserSync
		watchOptions: {
